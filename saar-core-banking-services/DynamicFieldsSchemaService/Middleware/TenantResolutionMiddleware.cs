namespace DynamicFieldsSchemaService.Middleware
{
    public class TenantResolutionMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            // 1. Primary: JWT claim (tamper-proof)
            var claim = context.User.FindFirst("tenant_id");
            if (claim is not null && !string.IsNullOrWhiteSpace(claim.Value))
                tenantId = claim.Value;

            // 2. Fallback: X-Tenant-ID header (service-to-service / dev testing)
            ...
            // Allowlist guard — unknown tenants fall back to "public"
